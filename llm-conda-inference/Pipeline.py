...
myenv = Environment(name="inferencellmenv")
run_config = RunConfiguration(conda_dependencies=conda)
run_config.environment.docker.enabled = True

exp = Experiment(workspace=ws, name="AML-INFERENCE-LLM-V1")
step_1 = PythonScriptStep(source_directory='C:/pythonprojects/aml/llm-conda-inference',
                          script_name="inferenceScript.py",
                          compute_target=compute_name,
                          runconfig=run_config,
                          allow_reuse=False)

pipeline = Pipeline(workspace=ws, steps=[step_1])
pipeline_run = pipeline.submit("AML-INFERENCE-LLM-V1", regenerate_outputs=True)